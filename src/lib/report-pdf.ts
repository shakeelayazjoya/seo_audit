      ...
      <head>
        ...
        <style>
          * { box-sizing: border-box; }
          ...
          .module, .top-issues { background: white; border-radius: 18px; padding: 24px; margin-top: 24px; box-shadow: 0 10px 30px rgba(15, 23, 42, 0.06); }
...
export async function getAuditReportData(auditId: string) {
  ...
  if (!modules) {
    throw new Error('Audit modules unavailable');
  }

  return { audit, modules };
}
